...
function Scene:update(dt)
    ...
    for i, player in ipairs(self.game.players) do
        ...
        if self.playerStates[i].puckCooldown > 0 then
            self.playerStates[i].puckCooldown = self.playerStates[i].puckCooldown - dt
            goto continue
        end
    ...
    for i, cell in ipairs(self.cells) do
        if cell.currentOwner ~= nil and cell.ownerSince > 0 then
            self.playerStates[cell.currentOwner].up = self.playerStates[cell.currentOwner].up + 1
        end
    end
    ...
    for _, line in ipairs(lines) do
        ...
        for _, cellIndex in ipairs(line) do
            ...
            elseif lineOwner == nil then
                lineOwner = cell.currentOwner
            elseif lineOwner ~= cell.currentOwner then
                hasLine = false
                break
            end
        end
    ...
    for i = 1, #self.game.players do
        ...
        -- Only update score if game not won
        if not self.gameWon then
            self.playerStates[i].score = self.playerStates[i].score + self.playerStates[i].up * dt * 0.5
        end
    end
...
function Scene:draw()
    ...
    for i, player in ipairs(self.game.players) do
        ...
        if puckImage then
            ...
            local score = math.min(100, self.playerStates[i].score or 0)
            ...
            local miniAlpha = 1
            if self.gameWon and self.winningPlayer == i then
...
function Scene:load()
    ...
    self.cells = {
        { pucks = {}, currentOwner = nil, ownerSince = 0 },
        { pucks = {}, currentOwner = nil, ownerSince = 0 },
        { pucks = {}, currentOwner = nil, ownerSince = 0 },
        { pucks = {}, currentOwner = nil, ownerSince = 0 },
        { pucks = {}, currentOwner = nil, ownerSince = 0 },
    ...
    for i, player in ipairs(self.game.players) do
        ...
        elseif numPlayers == 2 then
            puckX = (i == 1) and puckLeft or puckRight
        else
            -- For 3+ players, distribute equally between puckLeft and puckRight
            puckX = puckLeft + (puckRight - puckLeft) * (i - 1) / (numPlayers - 1)
        end
